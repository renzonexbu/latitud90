<template>
    <div class="min-h-screen flex items-center justify-center bg-gray-50">
        <div class="text-center">
            <!-- Spinner animado -->
            <svg
                class="animate-spin h-12 w-12 text-teal-600 mx-auto mb-4"
            ...
            <div
                v-if="gatewayType === 'khipu' && attempt > 0"
    ...
    try {
        ...
        if (data) {
            if (data.status === "approved") {
                ...
                const finalUrl = props.rut
                    ? `${successUrl}${separator}rut=${encodeURIComponent(
                          props.rut
                      )}`
                    : successUrl;
            ...
            if (
                ...
                const failureUrl = `/payment/failure/${
                    props.orderDetailId
                }?status=${data.status}&message=${encodeURIComponent(
                    data.message || "Pago no pudo ser procesado"
                )}`;
                const separator = failureUrl.includes("?") ? "&" : "?";
                const finalUrl = props.rut
                    ? `${failureUrl}${separator}rut=${encodeURIComponent(
                          props.rut
                      )}`
                    : failureUrl;
                window.location.href = finalUrl;
            ...
            if (data.status === "pending_validation") {
                const failureUrl = `/payment/failure/${
                    props.orderDetailId
                }?status=pending_validation&message=${encodeURIComponent(
                    data.message || "Pago pendiente de validación"
                )}`;
                const separator = failureUrl.includes("?") ? "&" : "?";
                const finalUrl = props.rut
                    ? `${failureUrl}${separator}rut=${encodeURIComponent(
            ...
            if (data.status === "pending") {
                const failureUrl = `/payment/failure/${
                    props.orderDetailId
                }?status=pending&message=${encodeURIComponent(
                    data.message || "Pago pendiente"
                )}`;
                const separator = failureUrl.includes("?") ? "&" : "?";
                const finalUrl = props.rut
                    ? `${failureUrl}${separator}rut=${encodeURIComponent(
            ...
            console.log(
                "[PaymentSpinner] Estado no reconocido:",
                data.status,
                "redirigiendo a fallo"
            );
            const failureUrl = `/payment/failure/${
                props.orderDetailId
            }?status=unknown&message=${encodeURIComponent(
                data.message || "Estado de pago no reconocido"
            )}`;
            const separator = failureUrl.includes("?") ? "&" : "?";
            const finalUrl = props.rut
                ? `${failureUrl}${separator}rut=${encodeURIComponent(
        ...
        const separator = failureUrl.includes("?") ? "&" : "?";
        const finalUrl = props.rut
            ? `${failureUrl}${separator}rut=${encodeURIComponent(props.rut)}`
            : failureUrl;
        window.location.href = finalUrl;
    } finally {
...
onMounted(() => {
    ...
    if (!props.gatewayType || props.gatewayType === "unknown") {
        console.error(
            "[PaymentSpinner] Tipo de pasarela no válido:",
            props.gatewayType
        );
