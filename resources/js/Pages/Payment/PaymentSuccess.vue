...
const downloadReceipt = () => {
  ...
  if (!props.paymentData.order_detail?.id || !props.paymentData.payment?.id) {
    alert('No se encontraron los datos necesarios para generar el comprobante')
    return
  }

  // Crear URL con parámetros
  const url = `/payment/receipt/download?order_detail_id=${props.paymentData.order_detail.id}&payment_id=${props.paymentData.payment.id}`
