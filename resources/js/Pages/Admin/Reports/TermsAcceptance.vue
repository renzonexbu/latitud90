                ...
                <div v-if="termsAcceptanceData && termsAcceptanceData.data && termsAcceptanceData.data.length > 0">
                    ...
                    <div class="mt-6 flex flex-row items-center justify-end gap-4 relative">
                        ...
                        <div class="flex flex-row gap-1.5 items-center justify-center">
                            <div
                                @click="termsAcceptanceData.current_page > 1 && goToPage(termsAcceptanceData.current_page - 1)"
                                :class="['flex w-8 h-8 items-center justify-center rounded-full transition-colors', termsAcceptanceData.current_page > 1 ? 'cursor-pointer bg-white border border-[#e5e7eb] text-[#6b7280] hover:border-[#1c4f4a]' : 'cursor-not-allowed bg-[#f3f4f6] text-[#d1d5db]']"
                            >
                            ...
                            <template v-for="item in visiblePages" :key="item.key">
                                <div v-if="item.type === 'page'" @click="goToPage(item.value)" :class="['flex w-8 h-8 items-center justify-center rounded-full cursor-pointer transition-colors', termsAcceptanceData.current_page === item.value ? 'bg-[#1c4f4a] text-white' : 'bg-white border border-[#e5e7eb] text-[#6b7280] hover:border-[#1c4f4a]']">
                                    <span class="text-sm font-medium">{{ item.value }}</span>
                                </div>
                                <div v-else class="flex w-8 h-8 items-center justify-center text-[#9ca3af] text-sm">...</div>
                            </template>
                            <div
                                @click="termsAcceptanceData.current_page < termsAcceptanceData.last_page && goToPage(termsAcceptanceData.current_page + 1)"
                                :class="['flex w-8 h-8 items-center justify-center rounded-full transition-colors', termsAcceptanceData.current_page < termsAcceptanceData.last_page ? 'cursor-pointer bg-white border border-[#e5e7eb] text-[#6b7280] hover:border-[#1c4f4a]' : 'cursor-not-allowed bg-[#f3f4f6] text-[#d1d5db]']"
                            >
...
const visiblePages = computed(() => {
    if (!props.termsAcceptanceData || !props.termsAcceptanceData.last_page) return [{ type: 'page', value: 1, key: 'page-1' }]
    const total = props.termsAcceptanceData.last_page
    const current = props.termsAcceptanceData.current_page
    const items = []
    if (total <= 7) {
