    ...
    <div class="flex flex-row items-center justify-end gap-4 relative">
        ...
        <div class="flex flex-row gap-1.5 items-center justify-center">
            ...
            <div
                ...
                :class="[
                    'flex w-8 h-8 items-center justify-center rounded-full transition-colors',
                    currentPage > 1
                        ? 'cursor-pointer bg-white border border-[#e5e7eb] text-[#6b7280] hover:border-[#1c4f4a]'
            ...
            <template v-for="item in visiblePages" :key="item.key">
                ...
                <div
                    v-else
                    class="flex w-8 h-8 items-center justify-center text-[#9ca3af] text-sm"
                >
                    ...
                </div>
            </template>
...
export default {
    ...
    computed: {
        ...
        visiblePages() {
            ...
            if (current <= 3) {
                // Cerca del inicio: 1 2 3 4 ... último
                for (let i = 2; i <= 4; i++) {
                    items.push({ type: 'page', value: i, key: `page-${i}` });
                }
